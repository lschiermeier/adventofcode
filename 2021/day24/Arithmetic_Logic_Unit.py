...
def findValid(backward):
    ...
    while not found_best:
        ...
        for x in (range(9,0,-1) if backward else range(1,10)):
            new_m = monad + str(x)
            ...
            if  new_state["z"] < 1000000:
                if not new_state in possible_states[d+1].values() and len(new_m) < 14:
                    ...
                    return winning_monad, winning_state
        monads = monads + new_monads

winning_monad, winning_state = findValid(True)
print(f"Result Part 1: {winning_monad}")
print(f"End State = {winning_state}")

winning_monad, winning_state = findValid(False)
print(f"Result Part 2: {winning_monad}")
print(f"End State = {winning_state}")
